...
func sm_align (qrr *read_record.FastaRecord, dbr *read_record.FastaRecord, m int, n int, g int) *sm_alignment {
	qrid := qrr.Id
	dbid := dbr.Id
	qrseq := qrr.Seq
	dbseq := dbr.Seq
...
func main() {
	...
	qr_records := read_record.Read_fasta(*query)
	var qr_record *read_record.FastaRecord
	if qr_records.Next() {qr_record = qr_records.Record()}
	db_records := read_record.Read_fasta(*db)
	for db_records.Next() {
