...
func mm_cache(mm mm_model, seq string) []float64 {
	...
	for i := mm.k; i < len(seq)-mm.k; i++ {
		idx := ntindex(seq, mm.k, i)
		if idx == -1 {
			score[i] = score[i-1]
		} else {
			score[i] = score[i-1] + mm.score[idx]
		}
	}
...
func ntindex(kmer string, k int, off int) int {
	idx := 0
	for i := 0; i < k; i++ {
		nt := string(kmer[off+i])
		if nt == "A" || nt == "a" {
			idx += int(math.Pow(4.0, float64(k - i -1)) * 0)
		} else if nt == "C" || nt == "c" {
			idx += int(math.Pow(4.0, float64(k - i -1)) * 1)
		} else if nt == "G" || nt == "g" {
			idx += int(math.Pow(4.0, float64(k - i -1)) * 2)
		} else if nt == "T" || nt == "t" {
			idx += int(math.Pow(4.0, float64(k - i -1)) * 3)
		} else {
			return -1
		}
	}
...
func read_mm(file string) mm_model {
	...
	for scanner.Scan() {
		...
		if f[0] == "%" {
			...
			mm.size = size
			score = append(score, make([]float64, size)...)
		} else if len(f) == 2 {
			kmer := f[0]
			if mm.k == 0 {
				mm.k = len(kmer)
			...
			idx := ntindex(kmer, len(kmer), 0)
			if idx == -1 {
				fmt.Printf("alphabet error in: %s\n", kmer)
			}
			score[idx] = log2score(prob)
		}
	}
...
func canonical(beg int, end int, seq string) bool {
	if seq[beg:beg+2]   != "GT" {return false}
	if seq[end-1:end+1] != "AG" {return false}
	return true
}
...
func gff_sites(seq string, file string, dons *[]int, accs *[]int, gtag bool) {
	...
	for gff.Scan() {
		...
		strd := f[6]
		if src != "RNASeq_splice" {continue}
		if strd != "+" {continue}
		if feat != "intron" {continue}
		if (gtag && !canonical(beg, end, seq)) {continue}
		if no_dup(beg, *dons) {*dons = append(*dons, beg)}
		if no_dup(end, *accs) {*accs = append(*accs, end)}
	}
	sort.Ints(*dons)
	sort.Ints(*accs)
}
...
func gtag_sites(seq string, beg int, end int, dons *[]int, accs *[]int) {
	for i := beg; i < end; i++ {
		if seq[i] == 'G' && seq[i+1] == 'T' {*dons = append(*dons, i)}
		if seq[i] == 'A' && seq[i+1] == 'G' {*accs = append(*accs, i+1)}
	}
}
	...
	for n := 1; n <= nsites; n++ {
		...
		for _, dsites := range dcombos {
			for _, asites := range acombos{
				...
				if short_exon(dsites, asites, seqlen, gen, emin) {continue}
...
func read_record(fasta string) (string, string) {
	var seq string
	var idn string
...
func main() {
	...
	// Find all isoforms
	idn, seq := read_record(*flag_fasta)
	...
	if apwm != "" {accpwm = read_pwm(apwm)} else {_ = accpwm}
	if dpwm != "" {donpwm = read_pwm(dpwm)} else {_ = donpwm}
	if elen != "" {exolen = read_len(elen)} else {_ = exolen}
	if ilen != "" {intlen = read_len(ilen)} else {_ = intlen}
	if emm  != "" {
		exomm = read_mm(emm)
	...
	for i, mrna := range iso.mrnas {
		if apwm != "" {iso.mrnas[i].score += score_apwm(accpwm, mrna) * wapwm}
		if dpwm != "" {iso.mrnas[i].score += score_dpwm(donpwm, mrna) * wdpwm}
		if elen != "" {iso.mrnas[i].score += score_elen(exolen, mrna) * welen}
		if ilen != "" {iso.mrnas[i].score += score_ilen(intlen, mrna) * wilen}
		if emm  != "" {iso.mrnas[i].score += score_emm(exomm, ecache, mrna) * wemm}
		if imm  != "" {iso.mrnas[i].score += score_imm(intmm, icache, mrna, donpwm, accpwm) * wimm}
